...
// SJS Runtime
    ...
    OENV.prototype = {
        createBinding: function (name, val) {
    ...
    ORV.prototype = {
        didThrow: function () {
            return isOniException(this.value);
        }
    };
    ...
    function OXN_Seq(exps, xc, env) {
        ...
        var me = this;
        globalScheduler.schedule(function () {
            me.cont(me, new ORV(null));
        });
    }
    ...
    function Eval(oen, bindings) {
        ...
        valueObject.cont = function (exiting_context, rv) {
            if (rv.didThrow()) {
                ...
                callAsync(function () {
                    throw "Uncaught Oni exception: '" + rv.value.tag + "', val:'" + rv.value.value + "'";
                });
            }
            else {
                this.setValue(rv);
    ...
    var Typeof = SLift(function (o) {
        return typeof o;
    });
    this["Typeof"] = Typeof;

    var Not = SLift(function (a) {
        return !a;
    });
    this["Not"] = Not;

    var Equals = SLift(function (a, b) {
        return a == b;
    });
    this["Equals"] = Equals;

    var NEquals = SLift(function (a, b) {
        return a != b;
    });
    this["NEquals"] = NEquals;

    var IsNull = SLift(function (obj) {
    ...
    var Car = SLift(function (lst) {
        return lst[0];
    });
    this["Car"] = Car;

    var Cdr = SLift(function (lst) {
        return lst.length > 1 ? lst.slice(1) : emptylist;
    });
    this["Cdr"] = Cdr;

    var Cadr = SLift(function (lst) {
        return lst[1];
    });
    this["Cadr"] = Cadr;

    var Caddr = SLift(function (lst) {
        return lst[2];
    });
    this["Caddr"] = Caddr;

    var Cadddr = SLift(function (lst) {
        return lst[3];
    });
    this["Cadddr"] = Cadddr;

    var ListRef = SLift(function (lst, i) {
        return lst[i];
    });
    this["ListRef"] = ListRef;

    var ListTail = SLift(function (lst, i) {
        return lst.length > i ? lst.slice(i) : emptylist;
    });
    ...
    function GetCadddr(v) {
        return Cadddr(Get(v));
    }

    this["GetCadddr"] = GetCadddr;

    var Add = SLift(function (a, b) {
        return a + b;
    });
    this["Add"] = Add;

    var Sub = SLift(function (a, b) {
        return a - b;
    });
    this["Sub"] = Sub;

    var Mul = SLift(function (a, b) {
        return a * b;
    });
    this["Mul"] = Mul;

    var Div = SLift(function (a, b) {
        return a / b;
    });
    this["Div"] = Div;

    var Random = SLift(function (ceil) {
    ...
    _AtomicVar.prototype.read = function () {
        return this.value;
    };
    _AtomicVar.prototype.write = function (value) {
        this.value = value;
    };
    _AtomicVar.prototype.transform = function (xform_func) {
        return this.value = xform_func(this.value);
    };

    var AtomicVar = {};
    AtomicVar.create = function (value) {
        return new _AtomicVar(value);
    };
    AtomicVar.Create = SLift(AtomicVar.create);
    AtomicVar.Read = SLift(function (v) {
        return v.read();
    });
    AtomicVar.Write = SLift(function (v, value) {
        v.write(value);
    });
    AtomicVar.Transform = SLift(function (v, xform) {
        return v.transform(xform);
    });
    ...
    Signal.Notify = SLift(function (s, val) {
        return s.notify(val);
    });
    Signal.Wait = ALift(function (cont, s) {
        return s.wait(cont);
    });
    ...
    var Barrier = {};
    Barrier.create = function (is_open) {
        return new _Barrier(is_open);
    };
    Barrier.Create = SLift(Barrier.create);
    Barrier.Open = SLift(function (b) {
        return b.open();
    });
    Barrier.Close = SLift(function (b) {
        return b.close();
    });
    Barrier.Pass = ALift(function (cont, b) {
        return b.pass(cont);
    });
    ...
    Channel.Put = ALift(function (cont, c, val) {
        return c.put(val, cont);
    });
    Channel.Collect = ALift(function (cont, c) {
        return c.collect(cont);
    });
    Channel.WaitAvail = ALift(function (cont, c) {
        return c.waitAvail(cont);
    });
    Channel.WaitEmpty = ALift(function (cont, c) {
        return c.waitEmpty(cont);
    });
...
// Always use push to add operands to an expression, to update start and end.
    ...
    function Statement(t, x) {
        ...
        // commons semicolon insertion magic after this switch.
        switch (tt) {
            case FUNCTION:
